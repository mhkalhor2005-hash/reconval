...
export async function GET(req: NextRequest) {
  ...
  if (user.role === "MANAGER") {
    ...
    if (repId) return NextResponse.json(await listRepInventory(Number(repId)));
    return NextResponse.json(await inventorySummaryAllReps());
  }
  return NextResponse.json(await listRepInventory(user.id));
}
...
export async function POST(req: NextRequest) {
  ...
  await allocateStock(Number(body.repId), Number(body.productId), Number(body.delta));
